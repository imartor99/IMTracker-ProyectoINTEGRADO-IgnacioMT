$(document).ready(function () {          //todo dentro de la funcion ready para asegurarnos que el DOM se carga antes que el JS
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $(nuevaFila).attr('id', `incidencia-${inc.id}`).removeClass().addClass(claseEstado);
    ...
    function actualizarFilaIncidencia(data) {
        ...
        if (rowIndex === undefined) {
            console.warn(`No se pudo obtener el índice para la fila con ID ${filaId}`);
            return;
        }
    ...
    function abrirModalDetalle(incidenciaId) {
        $.get(`/incidencia/${incidenciaId}/detalle/`, function (response) {
            if (response.success) {
                ...
                $('#detalleDescripcion').val(data.descripcion).prop('readonly', !data.puede_editar_descripcion);
                ...
                if (data.puede_cambiar_estado) {
                    ...
                    $('#detalleEstadoSelect').off('change').on('change', function () {
                        const estadoSeleccionado = $(this).val();
                        if (estadoSeleccionado === 'faltan_datos' || estadoSeleccionado === 'finalizada') {
                ...
                if (data.puede_editar_titulo || data.puede_editar_descripcion || data.puede_cambiar_estado) {
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    if (response.data) {
                        ...
                        if (typeof response.actualizar_contador !== 'undefined' && response.actualizar_contador) {
                            actualizarContador();
                        }
    ...
    $('#tablaIncidencias tbody').on('click', '.btnAsignar', function () {
        ...
        $.when(                                         //método de jQuery para esperar varias peticiones AJAX.
            $.getJSON('/obtener_usuarios_it/'),
            $.getJSON(`/obtener_incidencia/${incidenciaId}/`)
        ).done(function(usuariosData, incidenciaData) {                //se ejecuta cuando todas las peticiones en $.when(...) han tenido éxito.
            $('#selectUsuario').empty();
            ...
            if (usuarios.length === 0) {
                $('#selectUsuario').append('<option value="">No hay usuarios disponibles</option>');
            } else {
                usuarios.forEach(function (usuario) {
                    let nombreCompleto = (usuario.first_name + ' ' + usuario.last_name).trim() || usuario.username;
                    let seleccionado = (nombreCompleto === asignadoNombre) ? 'selected' : '';
                ...
                if (!asignadoNombre) {
                    $('#selectUsuario').prepend('<option value="" selected>Sin asignar</option>');  //prepend inserta contenido al principio de un elemento HTML o DOM.
                }
            }
